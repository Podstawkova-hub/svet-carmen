(function () {
  ...
  const SUPABASE_URL =
    "https://lggboavbzzvoomveewtl.supabase.co";

  const SUPABASE_KEY =
    "sb_publishable_3Z7UGlah1I0yJE7gHQQ57A_1XNEeauM";

  const likeBoxes =
    document.querySelectorAll(".carmen-like");
  ...
  /*
   * SPOLEČNÝ VZHLED PRO VŠECHNY STRÁNKY
   */
  ...
  style.textContent = `
    .carmen-like {
      ...
      box-shadow:
        0 15px 38px rgba(117, 58, 79, 0.10);
    }
    ...
    .carmen-like__button {
      ...
      box-shadow:
        0 10px 25px rgba(189, 73, 111, 0.24);
    ...
    .carmen-like__button:hover:not(:disabled) {
      ...
      box-shadow:
        0 14px 31px rgba(189, 73, 111, 0.31);
    }
    ...
    .carmen-like__button.is-liked {
      ...
      box-shadow:
        0 9px 23px rgba(189, 73, 111, 0.15);
    }
    ...
    .carmen-like__button.is-animating
    .carmen-like__heart {
      animation: carmenHeart 0.8s ease;
    }
  ...
  /*
   * DNEŠNÍ DATUM PRO OMEZENÍ
   * JEDNOHO SRDCE ZA DEN
   */
  ...
  async function callSupabase(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Supabase chyba ${response.status}: ${errorText}`
      );
    }
  ...
  async function createLikeBox(box) {
    ...
    const message =
      box.dataset.message ||
      "Pokud se vám tato stránka líbila, můžete jí dát své srdce.";
    ...
    box.innerHTML = `
      <p class="carmen-like__message"></p>
